...
// A preview leaves no lasting mark on the canvas, so only the deliberate toggle says
// which way it went. A hold is self-evident.
function toggleClientScripts() {
	...
	toast.info(canvasProps.scriptsRunning ? "Client scripts running" : "Client scripts stopped");
}

// Remembers the Run/Stop state from before a preview hold, so releasing the key restores
// it instead of always landing on Stop.
let scriptsRunningBeforeHold = false;

// One combo, two outcomes: a tap toggles Run/Stop, a hold previews with the scripts on
// and puts the earlier state back on release.
useShortcut({
	key: "p",
	ctrl: true,
	shift: true,
