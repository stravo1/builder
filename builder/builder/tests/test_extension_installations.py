...
class TestInstallationDetails(FrappeTestCase):
	...
	def test_the_document_answers_with_what_a_row_never_shows(self):
		installation = make_installation(
			EXTENSION,
			capabilities=["page.edit", "token.write"],
			granted=["page.edit"],
			readme="# Managed\n\nIt does one thing.",
		)
		...
		self.assertEqual(document.requested, ["page.edit", "token.write"])
		self.assertEqual(document.capabilities, ["page.edit"])
...
class TestGrantedCapabilities(FrappeTestCase):
	...
	def test_revoking_narrows_what_the_gate_allows(self):
		make_installation(EXTENSION, capabilities=["page.edit", "token.write"])

		set_granted_capabilities(EXTENSION, ["page.edit"])
	...
	def test_granting_again_reopens_it(self):
		make_installation(EXTENSION, capabilities=["page.edit", "token.write"], granted=["page.edit"])

		set_granted_capabilities(EXTENSION, ["page.edit", "token.write"])
	...
	def test_refuses_a_capability_the_manifest_never_asked_for(self):
		...
		with self.assertRaises(frappe.ValidationError):
			set_granted_capabilities(EXTENSION, ["page.edit", "schema.write"])
